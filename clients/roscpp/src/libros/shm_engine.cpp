...
ShmEngine::ShmEngine(const std::string& name,
                     const std::string& shmName, fiveai::platform::util::ByteCount shmSizeBytes) :
    m_name{name},
...
{
    using namespace boost::interprocess;
    using namespace fiveai::platform;
...
{
    return fiveai::platform::shm::convertToShared(allocateUniqueImage(sz), m_shmManager);
}
...
{
    const auto portableShmName = fiveai::platform::util::portableShmName(shmName);
...
{
    return fiveai::platform::util::portableShmQueueName(nodeName + "-shm-eng");
}
